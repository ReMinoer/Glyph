        ...
        where TMove : Move<TAction>
    {
        ...
        protected void AddSurroundingCases(Point parent, int limitMax, int limitMin = 1, int cond = 0)
        {
            ...
            foreach (Point move in list)
            {
                ...
                newNoeud.ParentCost = Closedlist[parent].ParentCost + parent.DistanceTo(newPoint);
                newNoeud.PersonalCost = newPoint.DistanceTo(Finish) + actionCost;
        ...
        protected virtual Point BestNodeClosedlist()
        {
            ...
            {
                float nd = n.Key.DistanceTo(Finish);
                if (nd < d)
                {
                    d = nd;
                    tmp = n.Key;
